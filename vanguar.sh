...
function mostrar_logo() {
    ...
    echo -e "${CYAN} ██  ██  ██   ██ ██  ██ ██ ██    ██ ██    ██ ██   ██ ██   ██ ██   ██${NC}"
    echo -e "${CYAN}  ████   ██   ██ ██   ████  ██████   ██████  ██   ██ ██   ██ ██████ ${NC}"
    echo -e "${BLUE}                   CLOUD ARCHITECTURE v5.3                            ${NC}"
    echo -e "${BLUE}======================================================================${NC}"
}
...
function iniciar_painel() {
    ...
    while true; do
        ...
        case "$opcao" in
            ...
            6)
                mostrar_logo
                echo -e "${YELLOW}=== PAREAMENTO E CONEXÃO ADB ===${NC}"
                ...
                if [ -n "$pairing_endpoint" ]; then
                    ...
                    if [ -n "$cod_par" ]; then
                        ...
                        if adb pair "$pairing_endpoint" "$cod_par" 2>&1 | grep -q "Successfully\|already paired"; then
                            ...
                            default_ip=$(echo "$pairing_endpoint" | cut -d':' -f1)
                            
                            if [ -n "$connection_endpoint" ]; then
                                echo -e "${CYAN}Conectando em $connection_endpoint...${NC}"
                                adb connect "$connection_endpoint" >/dev/null 2>&1
                            else
                                echo -e "${YELLOW}[!] mDNS não retornou a porta de conexão automática.${NC}"
                                echo -ne "Digite o IP e porta de CONEXÃO exibidos no topo (Ex: ${default_ip}:40077 ou 127.0.0.1:porta): "
                                read -r manual_conn < /dev/tty 2>/dev/null || read -r manual_conn
                                if [ -n "$manual_conn" ]; then
                                    adb connect "$manual_conn" >/dev/null 2>&1
                                fi
                            fi
                        else
                ...
                # Validação reforçada de conexão ADB com fallbacks automáticos
                if adb devices | grep -E -q "\bdevice\b"; then
                    echo -e "${GREEN}✓ Conectado! Aplicando otimizações via ADB...${NC}"
                else
                    if [ -n "$default_ip" ]; then
                        adb connect "${default_ip}" >/dev/null 2>&1
                    fi
                    adb connect 127.0.0.1 >/dev/null 2>&1
                fi
                ...
                if adb devices | grep -E -q "\bdevice\b"; then
                    ...
                    echo -e "${RED}Erro: Falha na conexão ADB. Certifique-se de digitar a porta de CONEXÃO correta que aparece na tela do seu celular após o pareamento.${NC}"
                fi
                perguntar_abrir_jogo
                ;;
            7)
