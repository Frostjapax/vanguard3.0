...
# --- FUNÇÕES DE PERSONALIZAÇÃO E EXTRAS (Adicionadas anteriormente) ---

function vibrar() {
    command -v termux-vibrate >/dev/null 2>&1 && termux-vibrate -d 25 2>/dev/null
}

function efeito_texto() {
    local texto="$1"
    local delay=0.012
    for (( i=0; i<${#texto}; i++ )); do
        printf "${CYAN}%s${NC}" "${texto:i:1}"
        sleep $delay
    done
    echo ""
}
...
# Função de logo original mantida[cite: 1]
...
# Obter Fingerprint Original[cite: 1]
...
# Sistema de log apontando para /sdcard/[cite: 1]
...
# --- SISTEMA VIP (Personalização reintegrada com seus logs) ---
...
# Tela de Carregamento original[cite: 1]
...
# Abrir Jogo Original[cite: 1] + Notificação
...
# Mostrar Specs (Original[cite: 1] + Temperatura)
...
# IA Vanguard Original Completa[cite: 1]
...
# Menu Híbrido (Original[cite: 1] + Opção 8 Ferramentas Extras adicionadas)
...
function executar_opcao() {
    case $1 in
        1)
            # Lista exata da Opção 1 Original[cite: 1]
            tela_carregamento
            ...
            ;;
        2)
            # Lista exata da Opção 2 Original[cite: 1]
            tela_carregamento
            setprop debug.performance.tuning 1 2>/dev/null
            setprop video.accelerate.hw 1 2>/dev/null
            ...
            ;;
        3)
            # Lista exata da Opção 3 Original[cite: 1]
            tela_carregamento
        ...
        4)
            ...
            ;;
        5)
            # Lista exata da Opção 5 Original[cite: 1]
            tela_carregamento
            ...
            ;;
        6)
            # Lista massiva e exata da Opção 6 Original[cite: 1]
            mostrar_logo
            ...
            ;;
        7)
            # vanguard_io original[cite: 1]
            vanguard_io_ai
            mostrar_menu
            ;;
        8)
            # Nova função embutida: Criar Widgets no Termux
...
# Inicialização Híbrida: Checa VIP e exibe Menu
checar_licenca_vip
mostrar_menu
EOF
